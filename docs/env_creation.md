...
The repository requests a source build of vLLM. An offline source build also needs a CUDA compiler, CMake, Ninja, build-system wheels, and the source archives of nested dependencies. If your cluster accepts a prebuilt vLLM wheel, prepare that wheel on a compatible machine and adjust `requirements.txt` accordingly.
