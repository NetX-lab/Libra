...
- Host used for the preflight: `aura-5`, aarch64
...
- `npu-smi`: `25.5.1`
- Python: `3.10.20`
- Environment: `/data/qianzhirong/envs/rl_mindspeed_260`
- Base site packages: `/data/qianzhirong/envs/rl_framework_py310`
...
SNAPSHOT_NAME=ascend_megatron_validated_20260805 \
  bash scripts/capture_ascend_megatron_environment.sh
```

The canonical validated snapshot is stored on shared storage at
`/data/qianzhirong/environment_snapshots/ascend_megatron_validated_20260805`, so
...
The rollout virtual environment currently lives at `/root/vllm_ascend_env`.
...
```bash
rsync -a /root/vllm_ascend_env/ HOST:/root/vllm_ascend_env/
rsync -a /root/.local/share/uv/python/cpython-3.11.13-linux-aarch64-gnu/ \
  HOST:/root/.local/share/uv/python/cpython-3.11.13-linux-aarch64-gnu/
...
```bash
cd /root/RL_Framework_npu
BASE_PYTHON=/data/qianzhirong/envs/rl_framework_py310/bin/python \
VENV_DIR=/data/qianzhirong/envs/rl_mindspeed_260 \
...
export TORCH_EXTENSIONS_DIR=/data_nv2/qianzhirong/torch_extensions
...
```bash
/data/qianzhirong/envs/rl_mindspeed_260/bin/python \
  -m torch.distributed.run --standalone --nproc_per_node=1 \
  examples/test_megatron_core_npu_runtime.py \
  --model /data/qianzhirong/models/Qwen3-0.6B \
  --max-seq-length 128
```
...
`/data/qianzhirong/runs/mcore_npu_preflight_0600m.log`.
...
`/data/qianzhirong/runs/mcore_npu_train_step_0600m.log`.
...
`/data/qianzhirong/vendor/mindspeed_260`:
