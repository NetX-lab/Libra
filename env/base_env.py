...
# from serve.request_call.lm_call import LMCallingConfig, ConcatedLMGenResult
# from profiler.chrome_trace import *
...
class CoTEnv(BaseEnv):
    ...
    def __init__(
        ...
        if reset:
            self.reset(trace,update_legal_action=False)

    def reset(self,trace, update_legal_action=True):
        ...
        if update_legal_action:
            ...
            while cnt < 3:
                cnt += 1
                try:
                    self._legal_actions, api_completion_token,generator_id = (
                        self.update_legal_actions(trace)
                    )
                    break
        ...
        info["generate_time"] = dur
        info['generator_id'] = generator_id
        return self.get_state(), info

    def step(self, action, trace , update_legal_action=True):
        ...
        if not (terminated or truncated) and update_legal_action:
            #cnt = 0
                #cnt += 1
            try:
                self._legal_actions, api_completion_token, generator_id = self.update_legal_actions(trace)
                info["api_completion_token"] = api_completion_token
                info['generator_id']= generator_id
            except NoLegalActionException as e:
                terminated = True
