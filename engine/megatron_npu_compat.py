...
def apply_megatron_npu_compatibility() -> None:
    """Redirect MCore's RNG tracker to Ascend's native RNG implementation."""
    ...
    def set_rng_state(new_state, device: int | str | torch.device = -1, graph_safe: bool = False):
        if graph_safe:
            raise RuntimeError("CUDA graph-safe RNG is unsupported on Ascend")
        if device == -1:
    ...
    # MCore's CudaRNGStatesTracker calls both these private helpers and the
    # public torch.cuda methods.  Keep the replacement process-local: each
    # torchrun worker has its own Python interpreter.
    ...
    # Some checkpoint and diagnostic modules are imported lazily after the
    # initial compatibility pass.  Translate only an explicitly requested
    # CUDA device; calls without a device retain normal PyTorch semantics.
    for factory_name in (
        "tensor",
        "as_tensor",
        "scalar_tensor",
    ...
    # Bridge's model construction ends with model_module.cuda(...), and some
    # conversion helpers use Tensor.cuda(...).  Both are pure device moves on
    # this path and have direct torch-npu equivalents.
    torch.nn.Module.cuda = module_cuda
    torch.Tensor.cuda = tensor_cuda

    # MCore 0.14 still embeds CUDA device and tensor-type string constants in
    # its otherwise device-independent optimizer, DDP and pipeline code.  A
    # scoped code-object rewrite keeps third-party files immutable while using
    # real torch.npu tensors, streams and events at runtime.
    ...
    from megatron.core.optimizer import clip_grads, distrib_optimizer, grad_scaler, optimizer
    ...
    # Cover every MCore module already imported by model/provider creation,
    # including diagnostics and architecture-specific helpers.
    for module_name, module in tuple(sys.modules.items()):
        ...
        try:
            ...
            # Extension-backed modules expose no replaceable Python code.
            continue
