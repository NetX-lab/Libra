...
class TestCausalPrefixTree(unittest.TestCase):
    ...
    def test_global_fallback_for_unseen_prompt(self):
        """Tool-state evidence transfers to a previously unseen prompt."""
        state = self._make_state("search", "small", "success")
        self.tree.insert(Trajectory("seen", [state], [8000]))

        node = self.tree.lookup_with_fallback("unseen", [state])
        self.assertIsNotNone(node)
        self.assertEqual(node.depth, 1)
        self.assertEqual(node.mean_remaining_length, 8000)
    ...
    def test_save_load(self):
        ...
        try:
            ...
            global_node = new_tree.lookup_with_fallback("new-prompt", traj.return_states)
            self.assertIsNotNone(global_node)
            self.assertEqual(global_node.remaining_lengths, [5000.0])
        finally:
            os.unlink(path)
...
class TestCMLFQScheduler(unittest.TestCase):
    ...
    def test_on_tool_return_migrates_for_unseen_prompt(self):
        """A learned tool-state continuation applies beyond its prompt id."""
        result = self.scheduler.schedule(input_tokens=1000, prompt_id="new-prompt")
        state = ToolReturnState(
            tool_type="search",
            payload_size_class=PayloadSizeClass.SizeLarge,
            execution_status=ExecutionStatus.ToolSuccess,
