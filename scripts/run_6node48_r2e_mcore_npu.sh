...
if [[ "$initial_placement_mode" == "configured" ]]; then
    ...
    [[ "${#train_hosts[@]}" -gt 0 && "${#rollout_hosts[@]}" -gt 0 ]] || {
        echo "configured placement requires at least one training and one rollout host" >&2
        exit 2
    }
    ...
    "$config_python" -c \
        'import json,sys; train=sys.argv[2].split(","); rollout=sys.argv[3].split(","); json.dump({"strategy":"configured","gpus_per_host":8,"train_gpus":8*len(train),"rollout_gpus":8*len(rollout),"train_hosts":train,"rollout_hosts":rollout},open(sys.argv[1],"w"),indent=2)' \
...
train_hosts, rollout_hosts = int(sys.argv[2]), int(sys.argv[3])
expected = {
    "train_gpus": train_hosts * 8,
    "rollout_gpus": rollout_hosts * 8,
    "num_nodes": train_hosts,
}
for key, value in expected.items():
    actual = int(config.get(key, -1))
    if actual != value:
        raise SystemExit(f"configured topology mismatch: {key}={actual}, expected={value}")
' "$effective_config" "${#train_hosts[@]}" "${#rollout_hosts[@]}"
...
if [[ "$reuse_rollout" != "1" ]]; then
    for rollout_host in "${rollout_hosts[@]}"; do
        ...
        NODE_PASSWORD="$NODE_PASSWORD" INTERNAL_SSH_TIMEOUT="${INTERNAL_SSH_TIMEOUT:-30}" \
            "$project_dir/scripts/internal_ssh.sh" "$rollout_host" -- \
            test -f "$rollout_project_dir/scripts/start_r2e_rollout_pool_npu.sh"
        NODE_PASSWORD="$NODE_PASSWORD" INTERNAL_SSH_TIMEOUT="${INTERNAL_SSH_TIMEOUT:-30}" \
            "$project_dir/scripts/internal_ssh.sh" "$rollout_host" -- \
            test -x /root/vllm_ascend_env/bin/python
        NODE_PASSWORD="$NODE_PASSWORD" INTERNAL_SSH_TIMEOUT="${INTERNAL_SSH_TIMEOUT:-30}" \
            "$project_dir/scripts/internal_ssh.sh" "$rollout_host" -- \
            test -f /data/qianzhirong/models/Qwen3-14B/config.json
    done
fi
