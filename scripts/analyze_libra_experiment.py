...
def _parse_rank0_logs(path: Path) -> list[dict[str, Any]]:
    ...
    log_paths = (
        sorted(path.rglob("training_rank0_*.log"))
        + sorted(path.rglob("driver_node_0.log"))
        + sorted(path.glob("*.out"))
    )
...
def load_run(name: str, path: Path) -> RunSummary:
    ...
    eval_paths = sorted(path.rglob("*eval*metrics*.jsonl"))
    for eval_path in eval_paths:
        run.eval_records.extend(_load_jsonl(eval_path))
    # A shared log directory can retain metrics from an earlier launch. Keep
    # one record per training step and use the newest occurrence.
    eval_by_step: dict[int, dict[str, Any]] = {}
    for record in run.eval_records:
        if "step" in record:
            eval_by_step[_safe_int(record.get("step"), -1)] = record
    if eval_by_step:
        run.eval_records = [eval_by_step[step] for step in sorted(eval_by_step)]
...
def summarize_run(run: RunSummary) -> dict[str, Any]:
    ...
    eval_rewards = [
        _safe_float(record.get("eval_reward_mean")) for record in run.eval_records
    ]
    eval_accuracies = [
        _safe_float(record.get("eval_accuracy")) for record in run.eval_records
    ]
    return {
        ...
        "eval_points": len(run.eval_records),
        "eval_reward_first": eval_rewards[0] if eval_rewards else 0.0,
        "eval_reward_last": eval_rewards[-1] if eval_rewards else 0.0,
        "eval_reward_delta": (
            eval_rewards[-1] - eval_rewards[0] if len(eval_rewards) >= 2 else 0.0
        ),
        "eval_accuracy_first": eval_accuracies[0] if eval_accuracies else 0.0,
        "eval_accuracy_last": eval_accuracies[-1] if eval_accuracies else 0.0,
        "eval_accuracy_delta": (
            eval_accuracies[-1] - eval_accuracies[0]
...
def render_report(runs: list[RunSummary]) -> str:
    ...
    lines = [
        ...
        "## Run Summary",
        "",
        "| Run | Steps | Eval points | Eval reward first | Eval reward last | Eval accuracy first | Eval accuracy last | Step time mean | Traj/s | Tokens/s | Planner applied | C-MLFQ trees |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for run in runs:
        s = summaries[run.name]
        lines.append(
            "| {name} | {steps} | {ep} | {erf} | {erl} | {eaf} | {eal} | {st} | {tr} | {tok} | {pa} | {ct} |".format(
                name=run.name,
                steps=s["steps"],
                ep=s["eval_points"],
                erf=_fmt(s["eval_reward_first"]),
                erl=_fmt(s["eval_reward_last"]),
                eaf=_fmt(s["eval_accuracy_first"]),
                eal=_fmt(s["eval_accuracy_last"]),
    ...
    for run in runs:
        ...
        if s["eval_points"]:
            direction = (
                "improved" if s["eval_reward_delta"] > 0 else "flat/regressed"
            )
            lines.append(
                f"- **{run.name}**: fixed-evaluation reward {direction}; "
                f"{s['eval_reward_first']:.4f} -> {s['eval_reward_last']:.4f} "
                f"(delta={s['eval_reward_delta']:.4f}), accuracy "
                f"{s['eval_accuracy_first']:.4f} -> {s['eval_accuracy_last']:.4f} "
                f"across {s['eval_points']} checkpoints."
            )
        else:
            direction = "improved" if s["reward_delta"] > 0 else "flat/regressed"
            lines.append(
                f"- **{run.name}**: no fixed-evaluation series was found; "
                f"training-sample reward was {direction}, "
                f"first-window={s['reward_first']:.4f}, "
                f"last-window={s['reward_last']:.4f}, delta={s['reward_delta']:.4f}."
            )
        lines.append(
            f"  Generation mean changed {s['gen_len_first']:.1f} -> "
            f"{s['gen_len_last']:.1f} tokens."
        )
    if len(runs) >= 2:
